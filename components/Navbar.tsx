...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="fixed top-0 z-[100] w-full bg-[#0f172a]/80 backdrop-blur-md border-b border-white/5 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span className={`text-lg md:text-xl font-black tracking-tighter text-white uppercase italic ${!isHome ? 'hidden xs:block' : 'block'}`}>
              Manisa Servis
            </span>
          </Link>
        </div>
        ...
        {/* Desktop Menu */}
        <div className="hidden lg:flex gap-6 text-xs font-bold uppercase tracking-widest text-slate-400">
          {navLinks.map((link) => (
        ...
        <button 
          ...
          className="md:hidden relative z-[110] p-2 text-white hover:bg-white/10 rounded-xl transition-colors focus:outline-none"
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 bg-[#0f172a] z-[90] flex flex-col items-center justify-center gap-12 transition-all duration-500 ease-in-out md:hidden ${isMenuOpen ? 'translate-y-0 opacity-100 pointer-events-auto' : '-translate-y-full opacity-0 pointer-events-none'}`}>
          <div className="flex flex-col items-center gap-8 w-full px-6">
            {navLinks.map((link, i) => (
              <Link 
                ...
                className={`text-2xl font-black text-white italic uppercase tracking-tighter hover:text-primary transition-all duration-500 text-center w-full whitespace-nowrap ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}
          ...
          <div className={`flex flex-col items-center gap-4 transition-all duration-700 delay-300 ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            <a 
              href={`tel:${contact.phoneCall}`} 
              className="flex items-center gap-3 bg-white text-black font-black py-5 px-12 rounded-2xl text-xl shadow-2xl active:scale-95 transition-transform"
            >
