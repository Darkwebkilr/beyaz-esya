    ...
    <footer id="iletisim" className="py-24 px-6 max-w-7xl mx-auto text-center border-t border-slate-100">
      ...
      <div className="mt-16 py-8 border-y border-slate-50 flex flex-wrap justify-center gap-x-8 gap-y-4">
        {brands.slice(0, 6).map((b) => (
          <Link key={b.slug} href={`/markalar/${b.slug}`} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-primary transition-colors">
            {b.name} Servisi
          </Link>
        ))}
      </div>
      ...
      <div className="mt-24 pt-12 border-t border-slate-100 text-center text-sm text-slate-400 font-bold uppercase tracking-[0.3em]">
        <p>© 2026 Manisa Teknik Servis - Evolution Ajans Tarafından Tasarlanmıştır</p>
      </div>
    </footer>
  );
}
