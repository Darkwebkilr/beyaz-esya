...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://manisamerkezservisi.com/markalar/${brand.slug}`,
    },
  };
}
    ...
    "@graph": [
      {
        ...
        url: `https://manisamerkezservisi.com/markalar/${brand.slug}`,
        priceRange: "$$",
        address: {
          "@type": "PostalAddress",
      ...
      <main className="pt-32 pb-24 px-6 max-w-6xl mx-auto">
          ...
          <header className="text-center lg:text-left order-2 lg:order-1">
            ...
            <h1 className="text-4xl md:text-7xl font-black text-foreground italic uppercase tracking-tighter leading-[0.9] mb-8 break-words">
              ...
              <span className="text-black text-lg md:text-2xl tracking-widest block mt-6 font-black uppercase italic">
                Profesyonel Onarım | {contact.phone}
              </span>
            </h1>
          ...
          <div className="prose prose-slate max-w-none">
            <div className="grid lg:grid-cols-3 gap-12 items-center">
              <div className="lg:col-span-2">
                <h2 className="text-2xl font-black text-foreground italic uppercase mb-6 tracking-tight">
                  {brand.name} Marka Cihazlarda Uzman Çözümler
                </h2>
                <p className="text-black leading-relaxed mb-4 font-medium italic">
                  {brand.name} markasının teknolojik üstünlüğüne ve karmaşık
                  yapısına hakim olan ekibimizle, Manisa genelinde en kapsamlı
                  teknik servis desteğini sunuyoruz. Buzdolabından bulaşık
                  makinesine, çamaşır makinesinden fırına kadar tüm {brand.name}{" "}
                  modellerinde orijinal yedek parça garantisi veriyoruz.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
              <div className="p-6 bg-white rounded-3xl border border-slate-50 shadow-sm">
                <h3 className="text-lg font-black text-foreground uppercase italic mb-3">
                  Teknolojik Onarım
                </h3>
                <p className="text-black/70 text-sm leading-relaxed font-bold">
                  {brand.name} cihazların elektronik kart tamiri, inverter motor
                  değişimleri ve sensör kalibrasyonları gibi hassas işlemleri
                  özel ekipmanlarımızla yerinde gerçekleştiriyoruz.
                </p>
              </div>
              <div className="p-6 bg-white rounded-3xl border border-slate-50 shadow-sm">
                <h3 className="text-lg font-black text-foreground uppercase italic mb-3">
                ...
                <p className="text-black/70 text-sm leading-relaxed font-bold">
                  Cihazınızın ömrünü korumak için sadece {brand.name}{" "}
                  standartlarına uygun, fabrikasyon onaylı parçalar kullanıyor
                  ve takılan her parçaya 1 yıl resmi servis garantisi
                  sağlıyoruz.
                </p>
              </div>
            </div>
          ...
          {[
            {
              icon: ShieldCheck,
              title: "1 Yıl Garanti",
              desc: `${brand.name} cihazınıza takılan tüm parçalarda resmi teknik destek garantisi.`,
            },
            {
              icon: Clock,
              title: "Aynı Gün Servis",
              desc: "Manisa'nın her noktasına aynı gün içerisinde hızlı ve kesin çözüm.",
            },
            {
              icon: Star,
              title: "Sertifikalı Kadro",
              desc: `${brand.name} teknolojileri konusunda eğitimli ve deneyimli teknik ekip.`,
            },
          ].map((item, i) => (
            <div
              key={i}
        ...
        <section className="mb-20">
          <h2 className="text-3xl font-black text-foreground italic uppercase mb-8 tracking-tight text-center">
            Hizmet Verdiğimiz {brand.name} Cihazlar
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((service, i) => (
              <Link
                ...
                className="p-6 bg-white border border-slate-100 rounded-2xl hover:bg-slate-50 hover:border-primary/30 transition-all flex items-center gap-4 group shadow-sm"
              >
                ...
                <span className="text-foreground font-bold uppercase italic tracking-wide group-hover:text-primary transition-colors">
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              {
                name: "Hakan Y.",
                comment: `${brand.name} buzdolabım soğutmuyordu, aynı gün içinde gelip sorunu çözdüler. Çok profesyoneller.`,
                rating: 5,
              },
              {
                name: "Sema Nur",
                comment:
                  "Bulaşık makinesi için çağırmıştım, hem çok hızlı geldiler hem de fiyatları çok uygundu.",
                rating: 5,
              },
            ].map((rev, i) => (
              <div
        ...
        <section className="bg-white border border-slate-100 p-10 md:p-16 rounded-[40px] shadow-2xl shadow-primary/10 text-center relative overflow-hidden mb-20">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-600 text-lg mb-10 max-w-2xl mx-auto font-medium">
              Arızayı yerinde tespit edelim, onayınızla hemen onaralım.
              Manisa&apos;nın her yerine bir telefon uzağınızdayız.
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <a
                href={`tel:${contact.phoneCall}`}
                className="flex items-center justify-center gap-3 bg-foreground text-white font-black py-5 px-12 rounded-2xl text-xl hover:bg-primary transition-all shadow-xl shadow-primary/20"
              >
